...
- [ ] T015 [P] Run `cd client && npm test -- --run` — confirm **all** client tests pass with zero failures and zero skipped tests; validates FR-010 (existing tests remain green) and requirements SC-001, SC-002, SC-004, SC-005, SC-006

- [ ] T016 [P] Run `cd server && npm test` — confirm **all** server integration tests pass with zero failures; validates FR-004 (color+brushSize transmitted and stored) and SC-003 (all participants see correct color/size)
